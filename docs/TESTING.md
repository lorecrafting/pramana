...
| Pramāṇa pilot preflight bookkeeping | `elixir bin/check_pilot_preflight.exs --validate`; readiness claims use `--ready --subject <exact-candidate-git-sha>` | Network/model/database-free. Validates the recorded gate/evidence shape and explicit candidate-revision binding; cannot establish substantive rights, evaluator or provider approval. |
