...
first, LLM only for the residual, always labelled — [the shared invariants](INVARIANTS.md) invariant #5.
...
- `date_start` / `date_end` — on `works`, **and populated since 2026-08-28**, so "modern" is
  ...
  the exact shape of doc failure [the shared invariants](INVARIANTS.md) § *Keeping the documentation true* exists for.
