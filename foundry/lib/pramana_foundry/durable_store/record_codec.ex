defmodule PramanaFoundry.DurableStore.RecordCodec do
  ...
  def normalize_bundle(value) do
    with {:ok, map} <- normalize_map(value),
         ...
         normalized <- %{"schema_version" => 1, "result" => result, "events" => events, "projections" => projections, "intents" => intents},
  ...
  def normalize(:claim, value) do
    with {:ok, map} <- normalize_map(value),
         ...
         true <- map["status"] in ~w(pending claimed issued unknown succeeded failed non_started cancelled),
         true <- plain_map?(map["value"]) do
      {:ok, map}
    else
      {:error, _reason} = error -> error
  ...
  def normalize(:reservation, value) do
    with {:ok, map} <- normalize_map(value),
         ...
         true <- map["status"] in ~w(available reserved consumed refunded released),
         true <- plain_map?(map["value"]) do
      {:ok, map}
    else
      {:error, _reason} = error -> error
  ...
  def materialize_result(candidate, committed_seq) when is_integer(committed_seq) and committed_seq >= 0 do
  ...
  def decode_bound(:result, bytes, %{schema_version: schema, disposition: disposition, reason_code: reason, committed_seq: seq}) do
  ...
  def decode_bound(:event, bytes, %{schema_version: schema, event_id: id, type: type}) do
    with {:ok, value} <- decode(:event, bytes),
         true <- value["schema_version"] == schema and value["event_id"] == id and value["type"] == type do
      {:ok, value}
    else
      {:error, reason} -> {:error, reason}
      _ -> {:error, :relational_binding_mismatch}
  ...
  def apply_projection(state, {carrier, projection}) do
    ...
    expected = case Map.get(state, key) do nil -> -1; prior -> prior.revision end

    if projection["expected_revision"] == expected and transition["revision"] == expected + 1 do
      {:ok, Map.put(state, key, %{revision: transition["revision"], last_event_id: carrier["event_id"], value: transition["value"]})}
    else
  ...
  def reconstruct(event_values, stored) do
    transitions =
      Enum.flat_map(event_values, fn event ->
        case event["payload"]["projection"] do
          nil -> []
          transition -> [{%{"event_id" => event["event_id"], "transition" => transition}, transition_to_projection(event, transition)}]
        end
      end)
  ...
  defp transition_to_projection(event, transition) do
    %{"expected_revision" => transition["revision"] - 1, "revision" => transition["revision"], "last_event_id" => event["event_id"]}
  end
  ...
  defp validate_projection_payload(payload) do
    if Map.has_key?(payload, "projection") do
      with {:ok, transition} <- normalize_map(payload["projection"]),
           :ok <- keys(transition, ~w(namespace entity_id revision value), ~w(namespace entity_id revision value)),
  ...
  defp result_semantics(map) do
    case {map["schema_version"], map["disposition"], map["reason_code"]} do
      {1, "accepted", nil} -> :ok
      {1, disposition, reason} when disposition in ["rejected", "blocked"] and is_binary(reason) and reason != "" -> :ok
      _ -> {:error, :invalid_result_semantics}
    end
  end
  ...
  defp normalize_map(value) when is_map(value) and not is_struct(value) do
    Enum.reduce_while(value, {:ok, %{}}, fn {key, item}, {:ok, acc} ->
      normalized_key = cond do is_binary(key) -> key; is_atom(key) -> Atom.to_string(key); true -> nil end

      cond do
        is_nil(normalized_key) or normalized_key == "" or not String.valid?(normalized_key) -> {:halt, {:error, :invalid_key}}
        Map.has_key?(acc, normalized_key) -> {:halt, {:error, :duplicate_field}}
        true -> {:cont, {:ok, Map.put(acc, normalized_key, item)}}
      end
    end)
  end
  ...
  defp nonempty(map, fields) do
    if Enum.all?(fields, &(is_binary(map[&1]) and map[&1] != "")), do: :ok, else: {:error, :invalid_identity}
  end
  ...
  defp lowercase_digest?(value) when is_binary(value),
    do: byte_size(value) == 64 and value == String.downcase(value) and Regex.match?(~r/\A[0-9a-f]+\z/, value)
