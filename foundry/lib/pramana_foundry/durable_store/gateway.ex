defmodule PramanaFoundry.DurableStore.Gateway do
  ...
  def handle_call({:transact, actor_id, command, proposal}, _from, state) do
    ...
    next_state =
      if storage_failure?(result), do: %{state | mode: :recovery, reason: result}, else: state
  ...
  def handle_call({:command, command_id}, _from, state) do
    {:reply, fetch_command(state.conn, command_id), state}
  end
  ...
  defp do_transact(conn, actor_id, command, proposal, fault) do
    with :ok <- validate_actor(actor_id),
         ...
         {:ok, canonical} <- Encoding.canonical(%{"actor_id" => actor_id, "command" => command}),
      ...
      case existing(conn, command_id, actor_id, digest) do
        ...
        {:error, :not_found} ->
          commit_bundle(conn, actor_id, command, canonical, digest, proposal, fault)
      ...
      case transaction_result do
        {:ok, result} ->
          if fault == :after_commit_before_reply,
            do: {:error, {:outcome_unknown, get(command, "command_id")}},
  ...
  defp insert_events(conn, events, command_id) do
    reduce_insert(events, fn event ->
      with {:ok, encoded} <- Encoding.json(event) do
        Database.execute(
          conn,
  ...
  defp insert_projections(conn, projections) do
    reduce_insert(projections, fn projection ->
      with :ok <- check_projection_revision(conn, projection),
           {:ok, encoded} <- Encoding.json(get(projection, :value)) do
        ...
        if expected == -1 do
          Database.execute(
            ...
            [
              ...
              get(projection, :revision),
              get(projection, :last_event_id),
              {:blob, encoded}
            ]
          )
        else
  ...
  defp check_projection_revision(conn, projection) do
    ...
    case Database.query(
           ...
           [get(projection, :namespace), get(projection, :entity_id)]
         ) do
  ...
  defp insert_intents(conn, intents, command_id) do
    reduce_insert(intents, fn intent ->
      with {:ok, encoded} <- Encoding.json(get(intent, :value)) do
        Database.execute(
          conn,
  ...
  defp insert_generations(conn, generations) do
    reduce_insert(generations, fn generation ->
      Database.execute(
        conn,
        "INSERT INTO ledger_generations(generation_id, parent_generation_id, schema_version, allocation, consumed) VALUES (?, ?, 1, ?, ?)",
        [
  ...
  defp insert_claims(conn, claims) do
    reduce_insert(claims, fn claim ->
      with {:ok, encoded} <- Encoding.json(get(claim, :value)) do
        Database.execute(
          conn,
  ...
  defp insert_reservations(conn, reservations) do
    reduce_insert(reservations, fn reservation ->
      with {:ok, encoded} <- Encoding.json(get(reservation, :value)) do
        Database.execute(
          conn,
  ...
  defp insert_result(conn, command_id, result, committed_seq) do
    durable = Map.put(result, key_for(result, :committed_seq), committed_seq)

    with {:ok, encoded} <- Encoding.json(durable),
         :ok <-
           Database.execute(
             conn,
  ...
  defp fetch_command(conn, command_id) do
    case Database.query(conn, "SELECT result FROM command_results WHERE command_id = ?", [
           command_id
         ]) do
  ...
  defp authoritative_content(conn) do
    tables =
      ~w(inputs commands command_results events projections effects claims ledger_generations reservations legacy_records)
