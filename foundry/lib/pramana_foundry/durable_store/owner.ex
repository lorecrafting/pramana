defmodule PramanaFoundry.DurableStore.Owner do
  ...
  def acquire(database_path, opts) when is_binary(database_path) do
    with {:ok, identity} <- PathIdentity.existing(database_path), do: acquire_identity(identity, opts)
  end
    ...
    with :ok <- PathIdentity.revalidate(identity),
      ...
      result =
        with :ok <- Sqlite3.execute(conn, "PRAGMA busy_timeout = 0"),
          ...
          {:ok, %__MODULE__{conn: conn, marker_path: marker_path, epoch: epoch, identity: identity}}
        end
  ...
  defp resolve_previous_marker(marker_path, evidence) when is_binary(evidence) and evidence != "" do
    case File.read(marker_path) do
      {:error, :enoent} -> :ok
