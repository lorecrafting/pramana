defmodule PramanaFoundry.DurableStore.Database do
    ...
    validate_bound_rows(
      ...
      fn [namespace, entity_id, schema, revision, last_event_id, bytes] ->
        with {:ok, value} <- decode_object(bytes),
             true <-
               Map.keys(value) |> Enum.sort() ==
                 ~w(entity_id expected_revision last_event_id namespace revision schema_version value),
    ...
    validate_bound_rows(
      ...
      fn [effect_id, schema, digest, status, bytes] ->
        with {:ok, value} <- decode_object(bytes),
             true <-
               Map.keys(value) |> Enum.sort() ==
                 ~w(effect_id request_digest schema_version status value),
  ...
  defp decode_body(bytes, kind) when is_binary(bytes) do
    ...
    cond do
      ...
      kind == :versioned and value["schema_version"] != 1 ->
