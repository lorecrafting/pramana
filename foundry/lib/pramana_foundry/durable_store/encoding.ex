defmodule PramanaFoundry.DurableStore.Encoding do
  ...
  defp encode(value) when is_integer(value) and value >= -9_223_372_036_854_775_808 and value <= 9_223_372_036_854_775_807,
  ...
  defp normalize(value) when is_map(value) do
    with {:ok, pairs} <- normalize_pairs(value),
         {:ok, normalized} <- map_ok(pairs, fn {key, item} ->
  ...
  defp normalize_pairs(map) do
    pairs =
      Enum.map(map, fn
        {key, value} when is_binary(key) -> if(ascii_key?(key), do: {key, value}, else: :invalid)
