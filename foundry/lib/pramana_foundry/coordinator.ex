defmodule PramanaFoundry.Coordinator do
  ...
  def init(opts) do
    ...
    with {:ok, %{projection: projection, state: recovered_state}} <-
      ...
      recovered_state = recover_inflight_agents(recovered_state, events, event_log_path, adapter, herdr_timeout)
  ...
  def handle_call({:enqueue_ticket, ticket}, _from, %{state: state, event_log_path: event_log_path} = data) do
    ...
    case CoordState.enqueue_ticket(state, ticket) do
      {:ok, new_state} ->
        ...
        Checkpoint.append(event_log_path, "ticket_enqueued", task_id, "pending", "system", %{
          "ticket" => ticket,
  ...
  def handle_call({:receive_handoff, task_id, handoff, opts}, _from, %{state: state, event_log_path: event_log_path} = data) do
    ...
    case CoordState.receive_handoff(state, task_id, handoff, opts) do
      {:ok, assignment, new_state} ->
        ...
        Checkpoint.append(event_log_path, "handoff_received", task_id, run_id, "developer", attrs)
            ...
            if is_pid(supervisor) and is_binary(checkout) and Tick.herdr_available?() do
              ...
              case DynamicSupervisor.start_child(supervisor, child_spec) do
                {:ok, pid} when is_pid(pid) ->
                  ...
                  state_with_run_id = put_in(new_state, ["assignments", task_id, "reviewer_run_id"], new_run_id)
      ...
      {:error, reason, new_state} ->
        Checkpoint.append(event_log_path, "handoff_received", task_id, run_id, "developer", %{
          "handoff" => handoff,
          "rejected" => true,
          "reason" => reason
        })
  ...
  def handle_call({:receive_review, task_id, review, opts}, _from, %{state: state, event_log_path: event_log_path} = data) do
    run_id = get_in(state, ["assignments", task_id, "run_id"]) || "pending"
    ...
    case CoordState.receive_review(state, task_id, review, opts) do
      {:ok, assignment, new_state} ->
        Checkpoint.append(event_log_path, "review_received", task_id, run_id, "reviewer", %{
          "review" => review,
          "verdict" => verdict,
      ...
      {:error, reason, new_state} ->
        ...
        Checkpoint.append(event_log_path, "review_received", task_id, run_id, "reviewer", attrs)
  ...
  def handle_call({:integrate, task_id, opts}, _from, %{state: state, event_log_path: event_log_path} = data) do
    run_id = get_in(state, ["assignments", task_id, "run_id"]) || "pending"
    commit = get_in(state, ["assignments", task_id, "candidate_commit"]) || get_in(state, ["assignments", task_id, "handoff", "commit"])

    # Write-ahead: record integration intent
    Checkpoint.append(event_log_path, "integration_started", task_id, run_id, "developer", %{
      "commit" => commit
    })

    case CoordState.integrate_candidate(state, task_id, opts) do
      {:ok, assignment, new_state} ->
        outcome_commit = Map.get(assignment, "commit", commit)

        Checkpoint.append(event_log_path, "integration_completed", task_id, run_id, "developer", %{
          "outcome" => "succeeded",
          "commit" => outcome_commit
        })

        # Integration succeeded — clean up any running agents for this task
        cleaned_data = cleanup_agents_for_task(data, task_id)
        {:reply, {:ok, assignment}, %{cleaned_data | state: new_state}}
      ...
      {:error, reason, new_state} ->
        ...
        # Integration failed — task is parked, clean up agents
        cleaned_data = cleanup_agents_for_task(data, task_id)
        {:reply, {:error, reason}, %{cleaned_data | state: new_state}}
    end
  end

  def handle_call({:apply_pm_proposals, proposals, opts}, _from, %{state: state, event_log_path: event_log_path} = data) do
    case PM.apply_proposals(state, proposals, opts) do
      {:ok, new_state} ->
        ...
        Checkpoint.append(event_log_path, "pm_proposal_created", task_id, run_id, "pm", %{
          "proposals" => proposals
        })
  ...
  @impl true
  def handle_info(:tick, %{state: state, poll_ms: poll_ms, agent_registry: agent_registry, event_log_path: event_log_path, max_launch_retries: max_launch_retries, herdr_adapter: adapter, herdr_timeout: herdr_timeout} = data) do
    ...
    LogStore.append(Map.get(data, :coordinator_log_path), %{
      "event" => "tick_start", "source" => "coordinator",
    ...
    if Map.get(state, "paused", false) || Map.get(state, "stop_requested", false) do
      ...
      try do
        ...
        admitted = Enum.count(log, fn
          {:admitted, _id} -> true
          {:launched, _id} -> true
          _ -> false
        end)

        launch_failures = Enum.count(log, fn
          {:launch_failed, _id, _reason} -> true
          _ -> false
        end)

        skipped = Enum.count(log, fn
          {:skipped, _id, _status} -> true
          _ -> false
        end)

        # Write tick_processed summary event
        Checkpoint.append(event_log_path, "tick_processed", "tick", "tick", "system", %{
        ...
        |> Enum.each(fn
          ...
          {:admitted, id} -> IO.puts("  tick: admitted #{id}")
        ...
        kind, error ->
          ...
          LogStore.append(clp, %{"event" => "tick_error", "source" => "coordinator",
            "kind" => kind, "error" => inspect(error), "queue_size" => length(queue)})
  ...
  def handle_info({:agent_launched, task_id, :ok, info}, %{state: state, event_log_path: event_log_path} = data) do
    ...
    LogStore.append(Map.get(data, :coordinator_log_path), %{
      "event" => "agent_launched", "source" => "coordinator",
      "task_id" => task_id, "pane" => info[:pane_id], "agent" => info[:agent_name]
    })

    run_id = get_in(state, ["assignments", task_id, "run_id"]) || "pending"

    Checkpoint.append(event_log_path, "pane_created", task_id, run_id,
      "developer", %{"pane_id" => info[:pane_id], "agent_name" => info[:agent_name]})
  ...
  def handle_info({:agent_launched, task_id, {:error, stage, reason}, _info}, %{state: state, event_log_path: event_log_path, max_launch_retries: max_retries} = data) do
    ...
    cond do
      new_retries <= max_retries ->
        ...
        Checkpoint.append(event_log_path, "launch_retried", task_id, run_id, "developer", %{
        ...
        new_state =
          ...
          |> put_in(["assignments", task_id, "error"], "launch_failed: #{stage}: #{inspect(reason)}")
          |> Map.update!("queue", fn q -> q ++ [task_id] end)

        LogStore.append(Map.get(data, :coordinator_log_path), %{
          "event" => "agent_launch_retry", "source" => "coordinator",
          "task_id" => task_id, "stage" => stage, "reason" => inspect(reason),
          "retry" => new_retries, "max" => max_retries
        })

        emit_telemetry(Map.get(data, :telemetry_path), "agent_launch", %{
          "task_id" => task_id, "phase" => "launch", "outcome" => "retry",
          "stage" => to_string(stage), "reason" => inspect(reason), "retry" => new_retries
        })
      ...
      true ->
        IO.puts("  Parking #{task_id} after #{new_retries} failed launch attempts (max #{max_retries})")

        # Write-ahead: record park event before state mutation
        Checkpoint.append(event_log_path, "launch_parked", task_id, run_id, "developer", %{
          "retry_count" => new_retries,
          "max_retries" => max_retries,
          "stage" => to_string(stage),
        ...
        new_state =
          ...
          |> put_in(["assignments", task_id, "launch_retries"], new_retries)
          |> put_in(["assignments", task_id, "error"], "max_launch_retries(#{max_retries}): #{stage}: #{inspect(reason)}")

        LogStore.append(Map.get(data, :coordinator_log_path), %{
          "event" => "agent_launch_parked", "source" => "coordinator",
          "task_id" => task_id, "stage" => stage, "reason" => inspect(reason),
          "retries" => new_retries, "max" => max_retries
        })

        emit_telemetry(Map.get(data, :telemetry_path), "agent_launch", %{
          "task_id" => task_id, "phase" => "launch", "outcome" => "parked",
          "stage" => to_string(stage), "reason" => inspect(reason), "retries" => new_retries
        })
  ...
  def handle_info({:agent_completed, task_id, run_id, reason, _info}, %{state: state, event_log_path: event_log_path, telemetry_path: telemetry_path} = data) do
    ...
    Checkpoint.append(event_log_path, "task_completed", task_id, run_id, "developer", %{
      "reason" => inspect(reason)
    })
    ...
    emit_telemetry(telemetry_path, "agent_completed", %{
      ...
      "phase" => "complete", "outcome" => "completed",
      "reason" => inspect(reason)
    })
  ...
  def handle_info({:agent_crashed, task_id, run_id, reason, info}, %{state: state, event_log_path: event_log_path} = data) do
    ...
    Checkpoint.append(event_log_path, "task_crashed", task_id, run_id, "developer", %{
      "reason" => inspect(reason),
    ...
    if new_retries <= max_work_retries do
      IO.puts("  Re-enqueueing #{task_id} after crash (work retry #{new_retries}/#{max_work_retries})")
      ...
      new_state =
        ...
        |> put_in(["assignments", task_id, "work_retries"], new_retries)
        |> put_in(["assignments", task_id, "error"], "agent_crashed(#{max_work_retries}): #{inspect(reason)}")

      emit_telemetry(Map.get(data, :telemetry_path), "agent_crash", %{
        "task_id" => task_id, "run_id" => run_id,
        "phase" => "crash", "outcome" => "parked",
        "reason" => inspect(reason)
      })
    ...
    if is_pid(supervisor) and is_binary(checkout) and is_map(handoff) and Tick.herdr_available?() do
      ...
      case DynamicSupervisor.start_child(supervisor, child_spec) do
        {:ok, pid} when is_pid(pid) ->
          ...
          state_with_run_id = put_in(new_state, ["assignments", task_id, "reviewer_run_id"], new_run_id)
          {%{data | agent_registry: Map.put(data.agent_registry, "#{task_id}-review", pid)}, state_with_run_id}
    ...
    case PramanaFoundry.Herdr.Runner.System.run(
      ...
      {:ok, %{stdout: stdout}} ->
        case :json.decode(stdout) do
          ...
          _ -> IO.puts("  cleanup: could not parse pane list")
        end
      {:error, reason} -> IO.puts("  cleanup: pane list failed: #{inspect(reason)}")
    end
  end
        ...
        if status not in ~w(dispatched) do
          ...
          case PramanaFoundry.Herdr.Runner.System.run(
            ...
            {:ok, %{stdout: stdout}} ->
              case :json.decode(stdout) do
                %{"result" => %{"pane" => _pane}} ->
                  read_inflight_pane(acc_state, event_log_path, task_id, run_id, pane_id, adapter, timeout_ms)
                _ ->
          ...
          handoff ->
            ...
            case PramanaFoundry.Coordinator.State.receive_handoff(state, task_id, handoff, []) do
              {:ok, _assignment, new_state} ->
                ...
                PramanaFoundry.Effects.Checkpoint.append(event_log_path,
                  "handoff_recovered", task_id, run_id, "developer",
  ...
  defp find_handoff_in_output(output) do
    ...
    |> Enum.find_value(:none, fn line ->
      ...
      if String.starts_with?(trimmed, "{") and String.ends_with?(trimmed, "}") do
        ...
        if is_map(decoded) and (Map.has_key?(decoded, "outcome") or Map.has_key?(decoded, "commit")) do
          decoded
        else
          :none
        end
  ...
  defp reenqueue_stale_dispatched(state, _events, event_log_path) do
    ...
    if all_recovered != [] do
      ...
      IO.puts("  re-enqueueing #{length(all_recovered)} recoverable ticket(s): #{inspect(stale_ids)}")

      updated =
        Enum.reduce(all_recovered, assignments, fn {tid, a}, acc ->
          run_id = Map.get(a, "run_id", "pending")

          event_name = if Map.get(a, "status") == "crashed", do: "task_crashed", else: "ticket_re_enqueued"
          Checkpoint.append(event_log_path, event_name, tid, run_id, "system", %{
            "reason" => if(Map.get(a, "status") == "crashed", do: "crash_recovered", else: "stale_dispatch_recovered"),
          ...
          |> Map.put("error", if(Map.get(a, "status") == "crashed", do: "crash_recovered", else: "stale_dispatch_recovered"))
  ...
  # ── Terminal-state pane cleanup ──

  defp cleanup_agents_for_task(data, task_id) do
    reg = data.agent_registry
    agents_to_kill = [task_id, "#{task_id}-review"]
    sup = Process.whereis(PramanaFoundry.AssignmentSupervisor)

    Enum.reduce(agents_to_kill, data, fn key, acc ->
      case Map.get(reg, key) do
        pid when is_pid(pid) ->
          IO.puts("  cleaning up agent #{key} for #{task_id}")
          DynamicSupervisor.terminate_child(sup, pid)
          %{acc | agent_registry: Map.delete(acc.agent_registry, key)}
        _ -> acc
      end
    end)
  end
