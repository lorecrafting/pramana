defmodule PramanaFoundry.Coordinator.State do
  ...
  def enqueue_ticket(state, ticket) when is_map(ticket) do
    ...
    cond do
      ...
      Map.has_key?(ticket, "auto_approve") ->
        {:error, "ticket auto_approve is forbidden; independent review is mandatory"}
      ...
      not validate_ticket_structure(ticket) ->
        {:error,
         "ticket has invalid structure: scope must be list, exclusions must be list, required_checks must be list of string lists"}
  ...
  defp validate_ticket_structure(ticket) do
    ...
    required_checks_ok? =
      case ticket["required_checks"] do
        nil -> true
  ...
  def admit_assignment(state, task_id, run_id, role, opts \\ %{}) do
    ...
    cond do
      ...
      auto_approve_requested?(assignment["ticket"]) ->
        {:error, "ticket auto_approve is forbidden; independent review is mandatory"}
  ...
  def receive_handoff(state, task_id, handoff, opts \\ []) do
    ...
    cond do
      ...
      auto_approve_requested?(assignment["ticket"]) ->
        {:error, "ticket auto_approve is forbidden; independent review is mandatory", state}
          ...
          {:error, reason} ->
            ...
            if new_retries <= max_handoff_retries do
              retried_assignment =
                ...
                |> Map.put(
                  "error",
                  "invalid handoff (retry #{new_retries}/#{max_handoff_retries}): #{reason}"
                )
              ...
              {:error, "maximum handoff retries (#{max_handoff_retries}) exceeded: #{reason}",
               new_state}
            end
        end
    end
  ...
  def integrate_candidate(state, task_id, opts \\ []) do
    ...
    {:error,
     "legacy integration is suspended before intent or effects; FR-13/FR-14 restore verified promotion",
     state}
  end
  ...
  defp auto_approve_requested?(ticket) when is_map(ticket),
    do: Map.has_key?(ticket, "auto_approve")

  defp auto_approve_requested?(_ticket), do: false
end
