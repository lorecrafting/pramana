...
|---|---|
| Developer, active attempt before any valid result | Keep the **same nonterminal attempt** and immutable base/spec/policy lineage; return ticket `developing → queued` with `resume_phase: developing` and reason `developer_launch_non_started`. Release launch resources. Release its checkout/conflict lease only after proving the checkout was never exposed or mutated, then reacquire/revalidate it before retry; otherwise retain the lease and block affected work. Below the infrastructure limit, queue a bounded developer retry. At the limit, ticket becomes `blocked(developer_launch_infrastructure)` while the attempt remains active and resumable. Exhaustion of current developer allocation instead makes the attempt terminal `exhausted` and ticket `exhausted` |
| Reviewer, frozen candidate awaiting review | Keep attempt and ticket `awaiting_review`, immutable candidate/check receipts and reviewer ownership. Close only the failed reviewer execution; never enter developer retry or correction. Release reviewer launch resources; retain candidate custody and candidate/check leases. Below the limit, return to the durable reviewer queue. At the limit, ticket becomes `blocked(reviewer_launch_infrastructure)` with `resume_phase: awaiting_review`; the same attempt/candidate remain resumable. Missing current reviewer allocation yields `blocked(reviewer_budget)` or `exhausted` under protected policy, without discarding or approving the candidate |
| PM planning execution | Keep the same objective/spec-planning owner; infer no proposal. Release launch-only resources. Below the PM limit, return to its PM queue; at the limit or without current allocation, block as `pm_launch_infrastructure` or `pm_budget`. Admit no ticket and create no objective allocation |
| Check, freeze/import, build, integration or activation worker | Preserve its candidate/deployment phase and verified inputs; apply that phase's existing infrastructure retry/block row. Release only proved-unused launch resources. A retry references the predecessor and consumes its finite role-specific infrastructure allowance; infer no successful check/build/ref/release receipt |
...
|---|---|
| Objective without admitted spec; broad steering | Durable objective and bounded PM reservation; PM proposal is evidence, not authority |
| draft; specific spec or valid PM create | Common admission validates full assignment/policy/budget allocation; queued or blocked with reason; malformed spec rejected |
| queued/blocked; PM amend/park | New future spec revision or explicit blocked state; active assignments never edited; amendment does not reset budgets |
| queued; dependencies/resources/profile/reservation eligible; no pause/drain/cancel | Create a fresh attempt unless R4a retained a resumable developer attempt; create its launch intent and enter developing. Pre-intent denial remains queued and consumes no start unit or infrastructure ordinal |
| developing; success artifact validates and freezes | Attempt candidate_frozen; ticket awaiting_review; seal productive capability/deadline generation; kernel requests developer close through broker immediately |
| developing; freeze/import infrastructure failure before valid candidate | Retain submitted bytes; bounded starts.check retry after owned worker closure; blocked(freeze_failure) when unavailable; unknown preserves lease. Git/scope validation failure is invalid submission, never a frozen result |
| candidate_frozen; developer exit/timeout/abnormal exit | Cleanup observation only; preserve frozen candidate. No new developer and no attempt failure |
| developing; sealed stream has no valid candidate, verified exit/timeout | Attempt terminal failed/timed_out; after cleanup queue fresh bounded attempt or exhaust; normal exit alone is not success |
| developing; valid blocked/partial result | Terminal blocked attempt, blocked ticket; close developer, no review. Resume/rescope requires explicit command and fresh attempt |
| any open submission phase; malformed result | Durable rejected submission, charge one validation action; further submission allowed only while stream open and budget remains; exhaustion closes execution and exhausts ticket |
| candidate_frozen; developer closed, check capacity eligible | checking attempt, awaiting_review ticket; schedule root-mandated check workers before reviewer, retaining immutable candidate |
| checking; all mandatory check receipts passed | awaiting_review attempt; queue independent reviewer; checks with explicit policy-empty set follow same guarded transition |
| checking; actual check assertion fails | Terminal needs_correction attempt; queue fresh developer after all check workers close, or blocked(drain)/exhausted; failed candidate never goes to approval |
| checking; tool/infrastructure failure or timeout | Preserve candidate, bounded new check-run reservation after cleanup; pending same phase, or blocked(check_infrastructure)/exhausted. Unknown check retains lease and blocks retry |
| awaiting_review; check receipts valid and reviewer capacity available | reviewing attempt/ticket, independent reviewer launch with its own reservation; R4a returns a proved non-start to this same candidate/role queue |
| reviewing; valid approved exact-candidate verdict | Close/seal reviewer; after verified close ready_to_integrate; no Git success inferred |
| reviewing; correction verdict | Terminal needs_correction attempt; close/seal reviewer, then queued fresh developer or blocked(drain)/exhausted; never re-prompt old developer |
| reviewing; rejected verdict | Terminal rejected attempt/ticket; cleanup pending separately; later work needs explicitly admitted revision |
| reviewing; sealed stream no valid verdict and reviewer crash/timeout | Preserve candidate, close reviewer then bounded new reviewer execution; developer ledger untouched; exhaust if unavailable |
| ready_to_integrate; current base/evidence/policy valid | integrating ticket/attempt; root integration intent, then R1 claim/issue |
| ready_to_integrate/integrating; accepted base moved before issuance | Terminal superseded_base attempt; fresh bounded rebase developer plus renewed checks/review; blocked under drain or budget exhaustion |
| integrating; successful ref receipt and prior role/check workers closed | integrated ticket; terminal integrated attempt; deployment is separate. Exit notifications cannot overwrite this |
| integrating; proved no ref change, command infrastructure failed | Same phase with bounded integration-effect retry after old issuer termination; or blocked(integration_failure). Conflict uses moved-base row; unknown blocks reconciliation |
| blocked; explicit resume or recorded dependency/resource recovery | Revalidate spec/control/policy and existing allocation; return to stored resume_phase; fresh attempt only if prior attempt terminal. Partial/rescope and explicit operator blocks require steering, not automatic unblocking |
| exhausted; authenticated reset grants eligible units and explicitly resumes | Keep old attempt terminal; queue a fresh developer attempt using retained evidence as context, with newly bound checks/review; block if that role lacks allocation; never reset prior consumption |
| integrated/rejected/cancelled; ordinary launch/result/completion command | Reject transition; preserve terminal facts. New work requires explicit admission linked to predecessor, with parent-funded allocation |
| nonterminal ticket; cancel requested | Set orthogonal control, cancel pending/unissued effects, request owned interrupts; hold phase/evidence while issued effects reconcile |
| cancel_requested; every owned session AND non-session claim terminal, cleanup reconciled | If no integration occurred: cancelled ticket and active attempt terminal cancelled; already terminal dispositions retained. If integration occurred: integrated and cancel_finalized(after_integration); suppress deployment |
