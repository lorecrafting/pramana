...
|---|---|---|---|
| CAS on every read revision | Core | `durable_store/gateway.ex` `check_expected_revisions/4` (`revision_conflict`); protected read sets in `durable_store/protected_primitives.ex` `complete_read_set/3` (`stale_read_set`). A command-level `root_ledger/<base64url id>/<generation>` key reads `root_ledgers` (`durable_store/authority.ex` `read({:revision, {:root_ledger, …}})`); the legacy `ledger/` key still reads `ledger_generations`. Core checks every key a command states; which root facts a decision states is the controller's (the kernel states the ledger on its exhaustion plan, whose `close_attempt` reads none) | `durable_store/review_corrections_test.exs` "read preconditions are authoritative and rejected decisions cannot mutate"; `workflow/decide_e2e_test.exs` "allocation returned after the exhaustion decision refuses it" |
...
| Reviewer independence: within one ticket attempt, an execution of a role shares no recorded principal (effect `issuer`, inbox actor, whole predecessor chain) with any execution of a role that `independent_of_roles` pairs it with, in the new operation's policy **or at any `(policy_id, policy_revision)` an effect of the attempt already pinned**; recorded principals until FR-15aB. Controller-only remainder: the scope is the controller-supplied `(ticket_id, attempt_id)`, and Core admits a reviewer relabelled into another open attempt, so "exact candidate" rests on the kernel's `require_active_attempt` in `review_planned` and FR-13 must re-check attempt identity at acceptance (I2). The declared read sets of `create_effect` and `append_inbox` omit the sibling effects, inboxes and policy history this reads; the reads are inside the commit transaction, so this is a read-set-honesty gap, not a stale read (I3) ([design](fr-08/FR08B-REVIEWER-INDEPENDENCE-DESIGN-2026-09-23.md)) | Core | `protected_primitives.ex` `principal_independence/6` with `pinned_policies/3`, called from `create_effect` and, on an execution's first append, from `append_inbox` via `inbox_independence/3` (`principal_not_independent`) | `durable_store/review_independence_test.exs` "a review issued by the producing principal is refused; another principal is admitted"; "a review whose predecessor chain holds the producing principal is refused"; "the requirement cannot be skipped by what the controller sends or when"; "a pairing dropped from the policy after the producer still binds the attempt"; "a review under a second policy without the pairing is still bound"; "a first inbox append after the pairing is dropped is still bound" |
...
| Exhaustion choice (which terminal disposition) | Controller | `workflow/kernel/software/dispositions.ex` `do_transition("attempt_settled", …)`; Core's `attempt_settlement` fact carries no disposition. The allocation read the choice rests on is CAS-bound only because `kernel/software/developer.ex` `allocation/5` states its `root_ledger/` key | `workflow/kernel_plan_test.exs` "close_attempt, attempt_settled, then cancellation_finalized" (plan shape only); `workflow/decide_e2e_test.exs` "allocation returned after the exhaustion decision refuses it" |
