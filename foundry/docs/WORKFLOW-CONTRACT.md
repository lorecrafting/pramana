...
| Reviewer independence: within one ticket attempt, an execution of a role shares no recorded principal (effect `issuer`, inbox actor, whole predecessor chain) with any execution of a role the operator policy's `independent_of_roles` pairs it with; recorded principals until FR-15aB ([design](fr-08/FR08B-REVIEWER-INDEPENDENCE-DESIGN-2026-09-23.md)) | Core | `protected_primitives.ex` `principal_independence/6`, called from `create_effect` and, on an execution's first append, from `append_inbox` via `inbox_independence/3` (`principal_not_independent`) | `durable_store/review_independence_test.exs` "a review issued by the producing principal is refused; another principal is admitted"; "a review whose predecessor chain holds the producing principal is refused"; "the requirement cannot be skipped by what the controller sends or when" |
