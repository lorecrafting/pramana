...
|---|---|
| Developer {R4a.01.f1}, active attempt before any valid result {R4a.01.f2} | Keep the **same nonterminal attempt** and immutable base/spec/policy lineage; return ticket `developing → queued` with `resume_phase: developing` and reason `developer_launch_non_started`. Release launch resources. Release its checkout/conflict lease only after proving the checkout was never exposed or mutated, then reacquire/revalidate it before retry; otherwise retain the lease and block affected work. Below the infrastructure limit, queue a bounded developer retry. At the limit, ticket becomes `blocked(developer_launch_infrastructure)` while the attempt remains active and resumable. Exhaustion of current developer allocation instead makes the attempt terminal `exhausted` and ticket `exhausted` |
| Reviewer {R4a.02.f1}, frozen candidate awaiting review {R4a.02.f2} | Keep attempt and ticket `awaiting_review`, immutable candidate/check receipts and reviewer ownership. Close only the failed reviewer execution; never enter developer retry or correction. Release reviewer launch resources; retain candidate custody and candidate/check leases. Below the limit, return to the durable reviewer queue. At the limit, ticket becomes `blocked(reviewer_launch_infrastructure)` with `resume_phase: awaiting_review`; the same attempt/candidate remain resumable. Missing current reviewer allocation yields `blocked(reviewer_budget)` or `exhausted` under protected policy, without discarding or approving the candidate |
| PM {R4a.03.f1} planning execution {R4a.03.f2} | Keep the same objective/spec-planning owner; infer no proposal. Release launch-only resources. Below the PM limit, return to its PM queue; at the limit or without current allocation, block as `pm_launch_infrastructure` or `pm_budget`. Admit no ticket and create no objective allocation |
| Check, freeze/import, build, integration or activation worker {R4a.04.f1} | Preserve its candidate/deployment phase and verified inputs; apply that phase's existing infrastructure retry/block row. Release only proved-unused launch resources. A retry references the predecessor and consumes its finite role-specific infrastructure allowance; infer no successful check/build/ref/release receipt |
...
|---|---|
| Objective without admitted spec {R4.01.f1}; broad steering {R4.01.f2} | Durable objective and bounded PM reservation; PM proposal is evidence, not authority |
| draft {R4.02.f1}; specific spec or valid PM create {R4.02.f2} | Common admission validates full assignment/policy/budget allocation; queued or blocked with reason; malformed spec rejected |
| queued/blocked {R4.03.f1}; PM amend/park {R4.03.f2} | New future spec revision or explicit blocked state; active assignments never edited; amendment does not reset budgets |
| queued {R4.04.f1}; dependencies/resources/profile/reservation eligible {R4.04.f2}; no pause/drain/cancel {R4.04.f3} | Create a fresh attempt unless R4a retained a resumable developer attempt; create its launch intent and enter developing. Pre-intent denial remains queued and consumes no start unit or infrastructure ordinal |
| developing {R4.05.f1}; success artifact validates and freezes {R4.05.f2} | Attempt candidate_frozen; ticket awaiting_review; seal productive capability/deadline generation; kernel requests developer close through broker immediately |
| developing {R4.06.f1}; freeze/import infrastructure failure before valid candidate {R4.06.f2} | Retain submitted bytes; bounded starts.check retry after owned worker closure; blocked(freeze_failure) when unavailable; unknown preserves lease. Git/scope validation failure is invalid submission, never a frozen result |
| candidate_frozen {R4.07.f1}; developer exit/timeout/abnormal exit {R4.07.f2} | Cleanup observation only; preserve frozen candidate. No new developer and no attempt failure |
| developing {R4.08.f1}; sealed stream has no valid candidate {R4.08.f2}, verified exit/timeout {R4.08.f3} | Attempt terminal failed/timed_out; after cleanup queue fresh bounded attempt or exhaust; normal exit alone is not success |
| developing {R4.09.f1}; valid blocked/partial result {R4.09.f2} | Terminal blocked attempt, blocked ticket; close developer, no review. Resume/rescope requires explicit command and fresh attempt |
| any open submission phase {R4.10.f1}; malformed result {R4.10.f2} | Durable rejected submission, charge one validation action; further submission allowed only while stream open and budget remains; exhaustion closes execution and exhausts ticket |
| candidate_frozen {R4.11.f1}; developer closed {R4.11.f2}, check capacity eligible {R4.11.f3} | checking attempt, awaiting_review ticket; schedule root-mandated check workers before reviewer, retaining immutable candidate |
| checking {R4.12.f1}; all mandatory check receipts passed {R4.12.f2} | awaiting_review attempt; queue independent reviewer; checks with explicit policy-empty set follow same guarded transition |
| checking {R4.13.f1}; actual check assertion fails {R4.13.f2} | Terminal needs_correction attempt; queue fresh developer after all check workers close, or blocked(drain)/exhausted; failed candidate never goes to approval |
| checking {R4.14.f1}; tool/infrastructure failure or timeout {R4.14.f2} | Preserve candidate, bounded new check-run reservation after cleanup; pending same phase, or blocked(check_infrastructure)/exhausted. Unknown check retains lease and blocks retry |
| awaiting_review {R4.15.f1}; check receipts valid {R4.15.f2} and reviewer capacity available {R4.15.f3} | reviewing attempt/ticket, independent reviewer launch with its own reservation; R4a returns a proved non-start to this same candidate/role queue |
| reviewing {R4.16.f1}; valid approved exact-candidate verdict {R4.16.f2} | Close/seal reviewer; after verified close ready_to_integrate; no Git success inferred |
| reviewing {R4.17.f1}; correction verdict {R4.17.f2} | Terminal needs_correction attempt; close/seal reviewer, then queued fresh developer or blocked(drain)/exhausted; never re-prompt old developer |
| reviewing {R4.18.f1}; rejected verdict {R4.18.f2} | Terminal rejected attempt/ticket; cleanup pending separately; later work needs explicitly admitted revision |
| reviewing {R4.19.f1}; sealed stream no valid verdict {R4.19.f2} and reviewer crash/timeout {R4.19.f3} | Preserve candidate, close reviewer then bounded new reviewer execution; developer ledger untouched; exhaust if unavailable |
| ready_to_integrate {R4.20.f1}; current base/evidence/policy valid {R4.20.f2} | integrating ticket/attempt; root integration intent, then R1 claim/issue |
| ready_to_integrate/integrating {R4.21.f1}; accepted base moved before issuance {R4.21.f2} | Terminal superseded_base attempt; fresh bounded rebase developer plus renewed checks/review; blocked under drain or budget exhaustion |
| integrating {R4.22.f1}; successful ref receipt {R4.22.f2} and prior role/check workers closed {R4.22.f3} | integrated ticket; terminal integrated attempt; deployment is separate. Exit notifications cannot overwrite this |
| integrating {R4.23.f1}; proved no ref change {R4.23.f2}, command infrastructure failed {R4.23.f3} | Same phase with bounded integration-effect retry after old issuer termination; or blocked(integration_failure). Conflict uses moved-base row; unknown blocks reconciliation |
| blocked {R4.24.f1}; explicit resume or recorded dependency/resource recovery {R4.24.f2} | Revalidate spec/control/policy and existing allocation; return to stored resume_phase; fresh attempt only if prior attempt terminal. Partial/rescope and explicit operator blocks require steering, not automatic unblocking |
| exhausted {R4.25.f1}; authenticated reset grants eligible units {R4.25.f2} and explicitly resumes {R4.25.f3} | Keep old attempt terminal; queue a fresh developer attempt using retained evidence as context, with newly bound checks/review; block if that role lacks allocation; never reset prior consumption |
| integrated/rejected/cancelled {R4.26.f1}; ordinary launch/result/completion command {R4.26.f2} | Reject transition; preserve terminal facts. New work requires explicit admission linked to predecessor, with parent-funded allocation |
| nonterminal ticket {R4.27.f1}; cancel requested {R4.27.f2} | Set orthogonal control, cancel pending/unissued effects, request owned interrupts; hold phase/evidence while issued effects reconcile |
| cancel_requested {R4.28.f1}; every owned session AND non-session claim terminal {R4.28.f2}, cleanup reconciled {R4.28.f3} | If no integration occurred: cancelled ticket and active attempt terminal cancelled; already terminal dispositions retained. If integration occurred: integrated and cancel_finalized(after_integration); suppress deployment |
