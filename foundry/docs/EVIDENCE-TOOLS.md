...
| **Clause coverage** | same file | How many of the contract's obligations does anything assert? Every outcome obligation carries an ID in the contract and must be in `@clauses` or `@uncited`, never both and never neither. **124 obligations: 63 asserted, 61 recorded uncited**, printed from the lists each run. The punctuation splitter it replaced hid 7 obligations, double-counted 7 spans, and left 4 stale entries. |
