...
| **Clause coverage** | same file | How many of the contract's obligations does anything assert? Every outcome obligation carries an ID in the contract and must be in `@clauses` or `@uncited`, never both and never neither. **124 obligations: 64 asserted, 60 recorded uncited**, printed from the lists each run. The punctuation splitter it replaced hid 7 obligations, double-counted 7 spans, and left 4 stale entries. |
| **From-cell classification** | same file | Does every from-state obligation have a refusal behind it? IDs are annotated into the contract's governing tables and parsed out at test time; each must be `{:guarded, atoms}` with atoms the kernel declares, `{:protected, why}`, `{:input, why}` (the conjunct names the event, not a precondition), `{:effect, why}` (a branch inside the effect body, accepted either way) or `{:unguarded, why}` — a recorded defect. **67 of 72 classified: 40 guarded, 18 input, 5 protected, 2 effect, 2 unguarded.** It proves a guard EXISTS, never that the named one is right, and never that a test exercises it. |
...
| **Live-count claims** | same file | Do the current-state documents state the numbers the artifact actually has? Each registered sentence in `EVIDENCE-TOOLS.md` and the evidence-reduction tickets is pinned to a count computed from `@from_obligations`, `@clauses` and `@uncited`. Two fatal modes: a wrong number, and a pattern that matches **nothing** — a reworded sentence is a check gone quiet, not a pass. Historical documents (this log, the frozen briefing, reviewers' findings) are deliberately excluded: checking them against today's artifact would demand they be falsified. |
...
- **Every mechanism here can only see a guard that exists.** Nothing detects a contract
  ...
  **32 rows carry 61 from-cell conjuncts and 0 of the 61 appear in the coverage number** — where 61
  ...
  and the ID set is parsed out at test time so a contract edit fails rather than drifts. **67 of the
  72 are classified so far** — row :467's, including its `{:unguarded, ...}` entry, which is what
  makes B3 countable — and the remaining 5 are held deliberately, because classifying one is a
