...
| FR-08B | Migrate every command ingress to one live/replay reducer | FR-08A | **In progress: FR-08A is satisfied in full, discharging B4. Subcommit 1 — the pure state and event contract answering B1 and the reducer-level half of B2 — has been independently reviewed five times and blocked five times. The first two blocks shared one finding shape, guards that refuse a reported counterexample while the R4 row stays unexpressible, which is why the third attempt replaced the method rather than the guards: R4 and R4a's rows are parsed from the contract and driven through the kernel, per [the row-driven design](fr-08/fr08b-row-driven-coverage.md). The third review judged that method sound and should be kept, and blocked on the mechanism being incomplete — its outcome side was unverified — plus eight further findings, all now closed. All 32 rows drive; four clauses stay recorded as partial pending the R4a allowance product. Five mechanisms now carry the evidence, per [mechanical evidence discipline](#mechanical-evidence-discipline), and an independent review of the evidence architecture has recommended reducing them to two — see the reduction note in that section. Subcommit 1 is closed and gate-validated at `e41fae73`, with the fourth and fifth reviews' findings discharged. The one reachable `@legal_pairs` violation the fifth review found is **fixed at `4fa3bc96` as its own candidate** rather than reopening subcommit 1 or riding on subcommit 2, gate-validated at 912 passed / 13 skipped / 1 excluded. That candidate was then blocked on review and superseded by `771d0d66`, which stores `integrating` as the resume target and leaves the attempt's phase alone, as the contract requires; an independent review on 2026-09-22 reached the same verdict and found no sibling branch with the same gap. B3's kernel scope landed at `ef0b3a0d` with review fix `fee10952` (gate green, 988 passed): planning refuses under pause, drain and pending cancel, discharging R4.04.f3. B3's protected items — settlement-to-execution/role binding and non-start settlement after a policy revision — remain, as do the `terminal_settlement_v1` and `reset_fact_v1` producers subcommit 3 needs; all three are protected code under R3 and wait on operator maintenance. The rest of subcommit 2 and subcommits 3–5 are outstanding** | F07, F16 |
...
work accumulated concrete hygiene evidence; it creates no FR-24. On 2026-09-22 the operator approved splitting it into slices FR-23a (hygiene, gated by a file rule instead of a ticket dependency) and FR-23b (decomposition and retirement, keeping FR-23's dependencies); the node count is unchanged. Every parent outcome, scope, acceptance paragraph and
...
**Split 2026-09-22 (operator-approved):** FR-23a and FR-23b are slices of this ticket; everything below binds both. The split, its file rule and its dependency argument are in [the split proposal](fr-23/FR-23-SPLIT-PROPOSAL-2026-09-22.md).
