...
| FR-08B | Migrate every command ingress to one live/replay reducer | FR-08A | **In progress: FR-08A is satisfied in full, discharging B4. Subcommit 1 — the pure state and event contract answering B1 and the reducer-level half of B2 — has been independently reviewed twice and blocked twice, both times with the same finding shape: guards that refuse the reported counterexample while the R4 row stays unexpressible. Both verdicts and their corrections are recorded in [the review findings](fr-08/fr08b-subcommit1-review-findings.md) and [the correction design](fr-08/fr08b-subcommit1-correction-design.md). The third attempt replaces the method rather than the guards: R4 and R4a's rows are now parsed from the contract and driven through the kernel, per [the row-driven design](fr-08/fr08b-row-driven-coverage.md). All 32 rows drive; five clauses are recorded as partial, four of which need the R4a limit product. Awaiting a third independent review. B3 and subcommits 2–5 are outstanding** | F07, F16 |
