...
- Candidate implementation revision: `c91a582b66b6a7f664f7685a463c2efd04026fce`.
- Candidate implementation tree: `cb257c9bce1436b5a7906e6f40da7cb76a69a7ff`.
...
- Failed independent v7 review: `review-v7.md`, SHA-256
  `d7a2f287b463d7abc2dd5ec72205b8849e759d2737d376cc8385546a71379c86`.
- Implementation response: `review-response-v7.md`, SHA-256
  `4ba1412d05960046fdd8926719967f902ec399b16292fe46deb988ce9aaaeca0`.

This candidate makes only the bounded V7-R1/V7-R2 correction and the review's two
documentation-whitespace suggestions. Completed projection absence now checks the indexed
retained carrier set, while transaction-internal pre-materialization remains explicit.
Projection and ledger dependency traversal now share complete bounded owner result,
sequence, event and effect semantics and return typed corruption for retained invalidity.
The v7 review's passing disposition for B3–B6 and the existing 18-table, reducer, staging,
schema and VFS behavior are preserved.

No coordinator plan/log, CLI, live daemon, provider, credential, deployment or activation
state was changed.
...
- `mix test test/pramana_foundry/durable_store test/pramana_foundry/legacy_persistence_containment_test.exs test/pramana_foundry/effects/checkpoint_test.exs --seed 9192`:
  exit 0; 91 passed.
- `mix test --seed 9193`: exit 2; 510/511 passed. The sole failure is the existing
  projections benchmark environment check because Python cannot import `tiktoken`. This is
  an explicit non-FR-07 environment exclusion, not a full-suite pass.

The three new permanent regressions reproduce the v7 review traces: a missing completed
projection with a retained carrier, impossible result watermarks reached through both
projection and ledger dependencies, and a rejected owner retaining an effect. They assert
typed corruption, recovery fencing and zero dependent-command rows.
...
61feb7b421480c46cc995b0371d64ede424cce81e98002ab9231f4c83b5c6316  foundry/lib/pramana_foundry/durable_store/authority.ex
...
0674a6ca176427fca211bdd67c3af5b69b4d53e2e30b34c6aaa6d1354f304f2d  foundry/test/pramana_foundry/durable_store/authority_test.exs
