...
| — | `apply/2` closure | **done** | Closed 2026-09-22 as a post-condition in `advance/2` (kernel property 6, `:malformed_post_state`), not the typed table the design sized — see the status block in [fr08b-closure-candidate-design.md](fr08b-closure-candidate-design.md) and the IMPLEMENTATION-LOG entry. `bin/closure_probe.exs` reports `accepted-but-malformed: 0` with its bound line unchanged. The original record follows. **Was:** Not a candidate here. Found by EV-3's harness on its first run: **at least 20** `(type, key)` pairs in 14 event types produce a state `well_formed?/1` rejects, which bricks the log. A bound, not a count — `bin/closure_probe.exs` prints what it cannot see each run. Quarantined and measured in `IMPLEMENTATION-LOG.md`; needs its own candidate. **Sized 2026-09-22:** every pair lands on one of **four** predicates the validator already applies, and `Event`'s `@payloads` is already a per-`(type, key)` table that names keys without typing them — so the candidate is a typed table plus one refusal site, not twenty guards owing twenty atoms, reachability entries and sweeps. The probe's `accepted-but-malformed` count at 0 is its acceptance test, necessary and not sufficient. Candidate design: [fr08b-closure-candidate-design.md](fr08b-closure-candidate-design.md) |
...
| — | EV-6 from-cell conjuncts | moderate | **Landed; reviewed twice, blocked twice, both answered** — [briefing](fr08b-ev6-ev2-review-briefing.md), [findings](fr08b-ev6-ev2-review-findings.md). The candidate reviewed was delta `16fc73db..dc9582b3`, where 63 of 72 were classified. 72 from-cell IDs are annotated into the contract, content preservation proved against `16fc73db`, and every ID must carry a disposition. How many are classified is in `@from_obligations` and `@from_unclassified`, which is where it is computed; restating a moving count in prose is what five review rounds on this branch were spent on. Outcome-cell IDs are EV-2's half of the same edit |
| — | EV-2 clause IDs | largest | **Landed; reviewed twice, blocked twice, both answered** — same candidate, same briefing and findings as EV-6. 124 outcome obligations annotated and consumed: `@clauses` and `@uncited` are ID sets that must exactly tile the contract, the punctuation splitter is deleted, and the count prints from the lists on every run and is deliberately not restated here |
