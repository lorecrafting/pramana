...
| — | EV-2 clause IDs | largest | **Landed; reviewed twice, blocked twice, both answered** — same candidate, same briefing and findings as EV-6. 124 outcome obligations annotated and consumed: `@clauses` and `@uncited` are ID sets that must exactly tile the contract, the punctuation splitter is deleted, and the count prints from the lists. **63 asserted, 61 recorded uncited** |
