...
| 2 | EV-6 from-cell conjuncts | moderate | **Mechanism landed.** 72 from-cell IDs annotated into the contract, content preservation proved against `16fc73db`, every ID must carry a disposition. 12 of 72 classified; 60 are a recorded ratchet. Outcome-cell IDs are EV-2's half of the same edit |
| 3 | EV-2 clause IDs | largest | **Done.** 124 outcome obligations annotated and consumed: `@clauses` and `@uncited` are ID sets that must exactly tile the contract, the punctuation splitter is deleted, and the count prints from the lists. 64 asserted, 60 recorded uncited |
