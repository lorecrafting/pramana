...
| — | EV-6 from-cell conjuncts | moderate | **Landed, awaiting independent review** — [briefing](fr08b-ev6-ev2-review-briefing.md), [findings](fr08b-ev6-ev2-review-findings.md), delta `16fc73db..dc9582b3`. 72 from-cell IDs annotated into the contract, content preservation proved against `16fc73db`, every ID must carry a disposition. 67 of 72 classified; 5 held with stated reasons. Outcome-cell IDs are EV-2's half of the same edit |
| — | EV-2 clause IDs | largest | **Landed, awaiting independent review** — same candidate and same briefing. 124 outcome obligations annotated and consumed: `@clauses` and `@uncited` are ID sets that must exactly tile the contract, the punctuation splitter is deleted, and the count prints from the lists. 64 asserted, 60 recorded uncited |
