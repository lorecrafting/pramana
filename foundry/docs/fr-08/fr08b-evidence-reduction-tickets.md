...
| — | ~~`apply/2` closure~~ | — | **Not a candidate here.** Found by EV-3's harness on its first run: **at least 20** `(type, key)` pairs in 14 event types produce a state `well_formed?/1` rejects, which bricks the log. A bound, not a count — `bin/closure_probe.exs` prints what it cannot see each run. Quarantined and measured in `IMPLEMENTATION-LOG.md`; needs its own candidate. **Sized 2026-09-22:** every pair lands on one of **four** predicates the validator already applies, and `Event`'s `@payload_keys` is already a per-`(type, key)` table that names keys without typing them — so the candidate is a typed table plus one refusal site, not twenty guards owing twenty atoms, reachability entries and sweeps. The probe's `accepted-but-malformed` count at 0 is its acceptance test, necessary and not sufficient; see the log |
