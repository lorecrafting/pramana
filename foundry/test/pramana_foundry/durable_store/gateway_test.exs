defmodule PramanaFoundry.DurableStore.GatewayTest do
  ...
  test "missing initialization and unknown store versions enter explicit recovery", %{path: path} do
    ...
    assert {:error, {:recovery_mode, :not_initialized}} = Gateway.transact(gateway, "actor", command("C0"), bundle("C0"))
    ...
    assert :ok = Database.execute(conn, "UPDATE metadata SET value = '99' WHERE key = 'event_version'")
  ...
  test "one commit atomically writes separately constrained authority rows", %{path: path} do
    ...
    assert {:ok, _result, :idempotent} = Gateway.transact(gateway, "operator", command, bundle("C1"))
    assert {:error, :idempotency_conflict} = Gateway.transact(gateway, "other", command, bundle("C1"))

    changed = put_in(command["payload"], %{"changed" => true})
    assert {:error, :idempotency_conflict} = Gateway.transact(gateway, "operator", changed, bundle("C1"))
  end
  ...
  test "commit before lost reply is recovered by same command retry", %{path: path} do
    ...
    assert {:error, {:outcome_unknown, "C3"}} = Gateway.transact(gateway, "operator", command, bundle("C3"))
  ...
  test "hard process exits on both commit boundaries have the documented durable result", %{
    ...
    for {boundary, status, expected_events} <- [{"before", 71, 1}, {"after", 72, 2}] do
      ...
      gateway = start_supervised!({Gateway, path: path})
      assert {:ok, %{"commands" => ^expected_events, "events" => ^expected_events}} = Gateway.counts(gateway)
  ...
  test "an actual read-only filesystem store fails closed without rewriting it", %{root: root, path: path} do
    assert :ok = Gateway.initialize(path)
  ...
  test "foreign-key, uniqueness and projection CAS failures roll back the entire bundle", %{path: path} do
    cases = [
      ...
      {"CAS", put_in(bundle("CAS")[:projections], [%{projection("CAS", "event-CAS") | expected_revision: 0, revision: 1}])}
    ]
  ...
  test "unknown proposal versions and candidate-supplied fields fail before SQL", %{path: path} do
    ...
    assert {:error, :unsupported_version} =
             Gateway.transact(gateway, "operator", command("V2"), %{bundle("V2") | schema_version: 2})

    assert {:error, :unknown_field} =
             Gateway.transact(gateway, "operator", command("SQL"), Map.put(bundle("SQL"), :sql, "DROP TABLE commands"))
  ...
  test "SQLite WAL/FULL and verified content backup survive reopen", %{root: root, path: path} do
    gateway = ready_gateway(path)
    assert {:ok, _result, :committed} = Gateway.transact(gateway, "operator", command("BACKUP"), bundle("BACKUP"))
  ...
  defp bundle(id) do
    ...
    %{
      ...
      intents: [%{schema_version: 1, effect_id: "effect-#{id}", request_digest: String.duplicate("a", 64), status: "pending", value: %{"operation" => "test"}}],
      ledger_generations: [%{schema_version: 1, generation_id: "generation-#{id}", parent_generation_id: nil, allocation: 1, consumed: 0}],
      claims: [%{schema_version: 1, claim_id: "claim-#{id}", effect_id: "effect-#{id}", writer_epoch: "epoch-1", status: "active", value: %{}}],
      reservations: [%{schema_version: 1, reservation_id: "reservation-#{id}", generation_id: "generation-#{id}", claim_id: "claim-#{id}", dimension: "starts.developer", units: 1, status: "reserved", value: %{}}]
    }
  end

  defp projection(id, event_id) do
    %{schema_version: 1, namespace: "kernel-v1", entity_id: "ticket-#{id}", expected_revision: -1, revision: 0, last_event_id: event_id, value: %{"status" => "ready"}}
  end
end
