...
defmodule PramanaFoundry.Workflow.R4CoverageTest do
  ...
  # ── The documents that state this mechanism's counts must state the live ones ────────
  #
  # Five independent review rounds on this branch. Not one blocked on wrong code. Every
  # block was a claim left standing after the thing it described moved - a disposition
  # count, a delta reference, a mechanism description - and round five's were introduced by
  # the commit fixing round four's. Everything above makes the ARTIFACT checkable; nothing
  # made the prose about the artifact checkable, and prose is where every block landed.
  #
  # So the prose gets a mechanism. Each entry below pins one sentence in one current-state
  # document to a number computed here. Two failure modes, both fatal:
  #
  #   * a claimed number that does not match the artifact
  #   * a pattern that matches NOTHING, which means the sentence was reworded or deleted
  #     and this check went quiet. A silent pass is how a claims-checker would decay into
  #     the thing it exists to prevent.
  #
  # WHAT IT DOES NOT COVER, so the next stale claim is not a surprise: only the counts
  # registered here, only in the two current-state documents, and nothing about commit
  # references, prose descriptions of behaviour, or comments in code. Two of round five's
  # three stale claims - a code comment contradicting an entry below it, and a briefing
  # half-updated - would still get through. This is the cheap slice, not the answer.
  #
  # HISTORICAL documents are deliberately excluded and must stay excluded:
  # IMPLEMENTATION-LOG.md records what was true when it was written, the review briefing is
  # frozen at the review point, and the findings are a reviewer's own words. Checking those
  # against today's artifact would demand they be falsified.
  @live_claim_docs [
    {"../../../docs/EVIDENCE-TOOLS.md", "EVIDENCE-TOOLS.md"},
    {"../../../docs/fr-08/fr08b-evidence-reduction-tickets.md",
     "fr08b-evidence-reduction-tickets.md"}
  ]

  describe "the current-state documents quote the live counts" do
    test "every registered claim matches the artifact, and every pattern still matches" do
      live = live_counts()

      text =
        Enum.map_join(@live_claim_docs, "\n", fn {path, _} ->
          Path.join(__DIR__, path) |> File.read!() |> String.replace(~r/\s+/, " ")
        end)

      wrong =
        for {re, keys} <- claim_patterns(),
            problem <- check_claim(text, re, keys, live),
            do: problem

      assert wrong == [],
             "current-state documents disagree with the artifact:\n" <>
               Enum.map_join(wrong, "\n", &"  #{&1}")
    end

    # Red control per rule 1. Both failure modes, because a checker that only catches wrong
    # numbers goes quiet the moment someone rewords the sentence carrying them.
    test "a wrong number and a vanished sentence are both reported" do
      live = live_counts()
      {re, keys} = hd(claim_patterns())

      good =
        "**#{live.classified} of 72 classified: #{live.guarded} guarded, #{live.input} input, #{live.protected} protected, #{live.effect} effect, #{live.unguarded} unguarded.**"

      assert check_claim(good, re, keys, live) == []

      tampered = String.replace(good, "#{live.classified} of 72", "#{live.classified + 1} of 72")
      assert [problem] = check_claim(tampered, re, keys, live)
      assert problem =~ "claims"

      assert [gone] = check_claim("no sentence like that here", re, keys, live)
      assert gone =~ "matches nothing"
    end
  end

  defp live_counts do
    dispositions = Enum.map(@from_obligations, fn {_id, d} -> elem(d, 0) end)
    outcome_ids = contract_outcome_ids()
    asserted = for {_row, entries} <- @clauses, {id, _q} <- entries, do: id

    %{
      total: length(contract_from_ids()),
      classified: map_size(@from_obligations),
      held: length(@from_unclassified),
      guarded: Enum.count(dispositions, &(&1 == :guarded)),
      input: Enum.count(dispositions, &(&1 == :input)),
      protected: Enum.count(dispositions, &(&1 == :protected)),
      effect: Enum.count(dispositions, &(&1 == :effect)),
      unguarded: Enum.count(dispositions, &(&1 == :unguarded)),
      obligations: length(outcome_ids),
      asserted: length(asserted),
      uncited: length(@uncited)
    }
  end

  defp claim_patterns do
    [
      {~r/\*\*(\d+) of 72 classified: (\d+) guarded, (\d+) input, (\d+) protected, (\d+) effect, (\d+) unguarded\.\*\*/,
       [:classified, :guarded, :input, :protected, :effect, :unguarded]},
  ...
  defp check_claim(text, re, keys, live) do
    case Regex.scan(re, text) do
      [] ->
        [
          "#{inspect(re.source)} matches nothing - the sentence it checks was reworded or deleted, " <>
            "so this check went quiet rather than failing"
        ]

      matches ->
        for [_whole | captured] <- matches,
            {value, key} <- Enum.zip(captured, keys),
            String.to_integer(value) != live[key],
            do: "#{key}: the document claims #{value}, the artifact has #{live[key]}"
    end
  end
