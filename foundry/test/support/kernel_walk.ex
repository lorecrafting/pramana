defmodule PramanaFoundry.Test.KernelWalk do
  ...
  **The proposals are derived from R4's rows, never from the reducer's guards.** A
  ...
  def deep(state, seed, steps) do
    walk(state, seed, steps, tickets: ["T1"], terminating_period: 40, background_period: 40)
  end
  ...
  def candidates(walk) do
    ...
    control(walk) ++
      objective(state) ++
      Enum.flat_map(walk.tickets, fn id ->
  ...
  defp ticket_level(tid, ticket) do
    [
      ...
      {"ticket_parked", tid,
       %{"ticket_id" => tid, "reason" => "dependency", "resume_phase" => ticket["phase"]}},
  ...
  defp per_check(tid, id, checks) do
    for check <- checks,
        status <- ~w(passed failed timed_out) do
      {"check_recorded", tid,
       %{
         "ticket_id" => tid,
         "attempt_id" => id,
  ...
  defp checkwork(tid, id, attempt, checks) do
    [
      {"checks_started", tid, %{"ticket_id" => tid, "attempt_id" => id}},
      {"check_planned", tid,
       %{
         "ticket_id" => tid,
         "attempt_id" => id,
  ...
  defp integration(tid, id, executions) do
    ...
    [
      ...
      for outcome <- ~w(ref_created no_ref_change) do
